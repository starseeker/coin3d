...
obol::PerspectiveCamera rotorCamera(const obol::Scene & scene)
{
    obol::ViewAllRequest viewRequest;
    viewRequest.viewportWidth = DEFAULT_WIDTH;
    viewRequest.viewportHeight = DEFAULT_HEIGHT;

    obol::CameraOrbitRequest orbitRequest;
    orbitRequest.camera =
        obol::CameraFraming::viewAllPerspective(scene, viewRequest);
    orbitRequest.azimuthRadians = kPi / 6.0f;
    orbitRequest.elevationRadians = kPi / 8.0f;
    return obol::CameraFraming::orbit(orbitRequest);
}

bool addWindmillFiles(obol::Scene & scene,
                      obol::SceneGroupId vaneGroup)
{
    char towerPath[512];
    char vanesPath[512];
    ...
    for (const char *dataDir : candidateDataDirs) {
        ...
        const obol::SceneObjectId tower =
            obol::SceneIO::addInventorFile(towerPath,
                                           scene,
                                           obol::Transform{},
                                           obol::RootSceneGroupId,
                                           getCoinHeadlessContextManager());
        if (tower == obol::InvalidSceneObjectId) {
            continue;
        }

        const obol::SceneObjectId vanes =
            obol::SceneIO::addInventorFile(vanesPath,
                                           scene,
                                           obol::Transform{},
                                           vaneGroup,
                                           getCoinHeadlessContextManager());
        if (vanes != obol::InvalidSceneObjectId) {
            printf("Loaded windmill geometry from %s\n", dataDir);
            return true;
        }

        scene.removeObject(tower);
    }
...
int main(int argc, char **argv)
{
    ...
    if (!addWindmillFiles(scene, vaneGroup)) {
    ...
    scene.setCamera(rotorCamera(scene));
