...
private:
    void ensureWorkers(unsigned int rows)
    {
        ...
        // A pool can grow between renders. Wait until each new worker has
        // sampled the current generation before run() advances it; otherwise
        // a late-starting worker can execute the preceding task and decrement
        // the next render's completion count twice.
        std::unique_lock<std::mutex> lock(mutex_);
        workersReady_.wait(lock, [this] {
            return readyWorkers_ == workers_.size();
        });
    }
    ...
    std::condition_variable workersReady_;
